...
def adjust_user_data_opened_pack(uuid, pack):
    try:
        ...
        update_query += f"inventory[${pack[0]}] = coalesce(inventory[${pack[0]}], 0) + 1, inventory[${pack[1]}] = coalesce(inventory[${pack[1]}], 0) + 1, inventory[${pack[2]}] = coalesce(inventory[${pack[2]}], 0) + 1, inventory[${pack[3]}] = coalesce(inventory[${pack[3]}], 0) + 1, inventory[${pack[4]}] = coalesce(inventory[${pack[4]}], 0) + 1"
